...
/**
 * A `StateMachine` is responsible for managing the connection state and expose a `Request => Future[Response]`
 * regardless of the number of actual request / response cycles required with a backend server.
 *
 ...
 *
 * `StateMachine`s have a public type parameter to represent the response type that it will eventually produce. They
 * also have an internal type parameter to represent its internal state representation.
 *
 ...
 * state. The "current state" must be maintained in whatever is encapsulating the connection (specifically, in
 * [[com.twitter.finagle.postgresql.ClientDispatcher]] for the finagle client.)
 *
...
trait StateMachine[+R <: Response] {
  ...
  /**
   * The initial transition to execute upon starting this particular machine. Normally, this will contain `Send` actions
   * to send messages to the Postgres backend.
   *
   * @return
...
object StateMachine {
  ...
  /**
   * Send multiple messages to the backend after transitioning to the new sate. This allows sending multiple messages in
   * a row without waiting for a message from the backend between each of them.
   */
  ...
  /**
   * Respond to the client's request. This effectively completes the client's request, but will not release the
   * connection yet.
   *
   * @param value
  ...
  /**
   * The result of applying a message to a particular state. This encapsulates the new state to transition to as well as
   * possible side effects to produce.
   */
  ...
  /**
   * Indicates that the state machine is finished and the connection may be released. Postgres uses the
   * [[ReadyForQuery]] message to indicate this to the client, so it is expected to be provided here. Also, if
   * completing the sate machine should also complete the client's response, then the value to produce can be provided
   * in the `response` field. If the client's promise has already been fulfilled, this will have no effect.
   *
   * @param ready
   *   the [[ReadyForQuery]] message from the backend confirming the connection's state.
   * @param response
   *   an optional value to used to fulfill the client's promise. If this is [[None]], then the [[Respond]] action must
   *   have been produced during the state machine's lifecycle to guarantee the client receives a response.
   */

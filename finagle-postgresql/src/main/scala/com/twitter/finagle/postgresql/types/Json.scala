 ...
 *
 * This is meant to allow json libraries to parse out json without incurring the cost of going through its `String`
 * representation.
 *
...
case class Json(value: Buf, charset: Charset) {
   ...
   *
   * This is necessary since several JSON libraries assume the underlying json is encoded in utf8. It's usually the
   * case, but when it's not, then it's important to fallback to using `jsonString` for parsing.
   */
